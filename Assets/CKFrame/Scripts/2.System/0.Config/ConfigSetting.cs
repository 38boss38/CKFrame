...
namespace CKFrame
{
    ...
    /// 包含所有的配置文件
    /// </summary>
    [CreateAssetMenu(fileName = "ConfigSetting", menuName = "CKFrame/ConfigSetting")]
    public class ConfigSetting : ConfigBase
    {
        ...
        public T GetConfig<T>(string configTypeName, int id) where T : ConfigBase
        {
            ...
            {
                throw new System.Exception("CK:配置设置中不包含这个Key:" + configTypeName);
            }

            //检查ID
            if (!configDic[configTypeName].ContainsKey(id))
            {
                throw new System.Exception($"CK:配置设置中{configTypeName}不包含这个ID:{id}");
            }
